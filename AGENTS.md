...
- **Package id is `specter`.** It reaches its Bitcoin backend over the LXC bridge, not `.startos` DNS. The **Select Node** action (`actions/selectNode.ts`) picks the backend — direct **Bitcoin RPC** (recommended) or an experimental **Spectrum Node** over electrs/Fulcrum — and writes the credentials/selection into `config.json` + the node file. `main.ts` then resolves the active backend's live bridge `host:port` through the `bridgeAddress` helper in `utils.ts` (chained `.const()`) and merges it into `bitcoin_core.json` / `spectrum_node.json` before the daemon starts. The dependency host ids and internal ports come from each dep's own exports (`btcRpcBinding` / `electrumBinding` in `utils.ts`, imported from `bitcoin-core-startos`, `electrs-startos`, and `fulcrum-startos`). The `.const()` restarts specter only when the selection changes or the dependency's assigned bridge port actually changes — not on a dependency update; while the dependency is absent the address resolves to a loopback placeholder (`127.0.0.1:<internal port>`) that heals automatically on install.
