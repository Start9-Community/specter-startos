...
This repo packages [Specter Desktop](https://github.com/cryptoadvance/specter-desktop) for StartOS.
...
Specter Desktop runs the `ghcr.io/cryptoadvance/specter-desktop` image pinned via `dockerTag` in `startos/manifest/index.ts`.

1. Bump `dockerTag` in `startos/manifest/index.ts` to `ghcr.io/cryptoadvance/specter-desktop:v<new version>`.
